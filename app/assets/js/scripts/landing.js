...
function updateSelectedServer(serv){
    ...
    ConfigManager.save()
    server_selection_button.innerHTML = '\u2022 ' + (serv != null ? serv.getName() : 'Aucun serveur selectionné')
    if(getCurrentView() === VIEWS.settings){
...
function asyncSystemScan(mcVersion, launchAfter = true){
    ...
    setLaunchDetails('Veuillez pantienter..')
    toggleLaunchArea(true)
    setLaunchPercentage(0, 100)
    ...
    sysAEx.on('message', (m) => {
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    'Aucune installation Java compatible trouvées',
                    `Vous avez besoin d\'une installation 64 bits de Java ${javaVer}. Voulez-vous que nous en installions une copie ?`,
                    'Installer Java',
                    'Installer manuellement'
                )
                setOverlayHandler(() => {
                    setLaunchDetails('Préparation au téléchargement de Java..')
            ...
            if(m.result === true){
                ...
                setOverlayContent(
                    'Unxpected Issue:<br>échec du téléchargement de Java',
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur pendant le lancement', err.message || 'Voir la console (CTRL + Shift + i) pour plus de détails.')
    })
                ...
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            ...
                            'Impossible de se connecter au serveur de fichiers. Assurez-vous que vous êtes connecté à l\'Internet et réessayez.'
                        )
                    } else {
                        showLaunchFailure(
                            'Erreur lors du téléchargement',
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Joue!')
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Could not find or load main class net.minecraft.launchwrapper.Launch') > -1){
                        ...
                        showLaunchFailure('Erreur pendant le lancement', 'Le fichier principal, LaunchWrapper, n\'a pas pu être téléchargé correctement. En conséquence, le jeu ne peut pas se lancer. Pour résoudre ce problème, désactivez temporairement votre logiciel antivirus et relancez le jeu. Si vous avez le temps, veuillez soumettre un problème et nous faire savoir quel logiciel antivirus vous utilisez. Nous allons les contacter et essayer de redresser les choses.')
                    }
                }
                ...
                try {
                    ...
                    loggerLaunchSuite.error('Error during launch', err)
                    showLaunchFailure('Erreur lors du lancement', 'Veuillez consulter la console (CTRL + Shift + i) pour plus de détails.')
